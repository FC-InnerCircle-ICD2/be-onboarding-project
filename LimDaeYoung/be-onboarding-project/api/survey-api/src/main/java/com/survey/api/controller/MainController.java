...
public class MainController {
    ...
    @PutMapping("/{surveyId}/update-period")
    @PutMapping("/{surveyId}:updatePeriod")
    @PostMapping("/update")
    public ResponseEntity<SurveyBaseResponse> update(
            @PathVariable("surveyId") long surveyId,
            @RequestBody SurveyUpdateForm survey,
    ...
    // react-query: 캐싱되는 HTTP Client
    // POST에서는 캐싱이 안되거나, 혹은 되더라도 꺼놓음.
    // 보통은 GET에 대해서만 캐싱을 함.
